...
export default function Payroll() {
  ...
  async function loadConnection() {
    setLoading(true)
    try {
      const { data, error } = await supabase.functions.invoke('adp-proxy', {
  ...
  return (
    <div style={s.page}>
      ...
      <p style={s.sub}>Connect your existing ADP Workforce Now account to sync approved timesheets to payroll automatically.</p>
...
function ConnectTab({ companyId, connection, onConnected }) {
  ...
  function connectADP() {
    openBrowser('mailto:support@postcommand.app?subject=ADP%20Workforce%20Now%20Setup&body=Please%20help%20me%20connect%20ADP%20Workforce%20Now%20to%20PostCommand.')
  }

  async function disconnect() {
    if (!window.confirm('Disconnect ADP Workforce Now? This will not cancel any active payroll runs.')) return
    setDisconnecting(true)
    await supabase.from('adp_connection').delete().eq('company_id', companyId)
  ...
  if (connection) return (
    <div style={s.card}>
      <div style={{ display:'flex', alignItems:'center', gap:'16px', marginBottom:'20px' }}>
        ...
        <div>
          <div style={{ fontFamily:'var(--font-condensed)', fontSize:'13px', fontWeight:700, color:'var(--color-success)', letterSpacing:'1px' }}>CONNECTED TO ADP WORKFORCE NOW</div>
          <div style={{ fontSize:'13px', color:'var(--text-primary)', marginTop:'2px' }}>{connection.company_name || 'ADP Company'}</div>
      ...
      <div style={{ padding:'16px', background:'var(--bg-surface)', borderRadius:'var(--radius-md)', marginBottom:'20px', fontSize:'13px', color:'var(--text-secondary)', lineHeight:1.6 }}>
        ADP Workforce Now handles all tax calculations, direct deposits, W-2 filing, and compliance. PostCommand syncs approved hours directly to your ADP payroll runs.
      </div>
      <button onClick={disconnect} disabled={disconnecting} style={{ ...s.danger, opacity:disconnecting?0.6:1 }}>
        <Icon name="x" size={14}/>{disconnecting?'DISCONNECTING...':'DISCONNECT ADP'}
      </button>
    </div>
  )
  ...
  return (
    <div style={s.card}>
      <div style={{ textAlign:'center', padding:'24px 0' }}>
        ...
        <div style={{ fontFamily:'var(--font-display)', fontSize:'22px', letterSpacing:'2px', color:'var(--text-primary)', marginBottom:'12px' }}>CONNECT ADP WORKFORCE NOW</div>
        <div style={{ fontSize:'14px', color:'var(--text-secondary)', lineHeight:1.7, maxWidth:'420px', margin:'0 auto 24px' }}>
          ADP Workforce Now is a leading payroll, HR, and benefits platform. Connect your existing ADP account to run payroll directly from PostCommand using verified timesheet hours.
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:'10px', maxWidth:'360px', margin:'0 auto 28px', fontSize:'13px', color:'var(--text-secondary)', textAlign:'left' }}>
          {['Automatic tax calculations and filings','Direct deposit to employee bank accounts','W-2 and 1099 generation at year end','Benefits and time-off administration','Compliance with state/federal regulations'].map(f => (
            <div key={f} style={{ display:'flex', alignItems:'center', gap:'8px' }}><Icon name="check" size={14} color="var(--color-success)"/>{f}</div>
          ))}
        </div>
        <button onClick={connectADP} style={s.btn}>
          <Icon name="link" size={16}/>CONNECT ADP ACCOUNT
        </button>
        <div style={{ marginTop:'16px', padding:'12px 16px', background:'var(--bg-surface)', borderRadius:'var(--radius-sm)', fontSize:'12px', color:'var(--text-secondary)', lineHeight:1.6, maxWidth:'420px', margin:'16px auto 0' }}>
          ADP integration requires credential setup by the PostCommand team. Contact{' '}
          <a href="mailto:support@postcommand.app" style={{ color:'var(--accent)', textDecoration:'none' }}>support@postcommand.app</a>{' '}
          to complete your ADP Workforce Now connection.
        </div>
      </div>
    </div>
  )
...
function RunPayrollTab({ companyId, connection, profile }) {
  ...
  async function submit() {
    if (!connection) { alert('Connect ADP Workforce Now first.'); return }
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('adp-proxy', {
        body: { action:'sync_timesheets', company_id:companyId, data:{ pay_period_start:payPeriod.start, pay_period_end:payPeriod.end, employees:payload } }
      })
      if (error) throw new Error(error.message)
      setResult({ ok:true, text: data?.message || 'Payroll submitted to ADP Workforce Now. Processing within 2 business days.' })
    } catch(e) {
  ...
  if (!connection) return (
    <div style={{ textAlign:'center', padding:'60px 24px', color:'var(--text-muted)' }}>
      ...
      <div style={{ marginTop:'16px', fontSize:'15px' }}>Connect ADP Workforce Now first</div>
      <div style={{ marginTop:'6px', fontSize:'12px' }}>Go to the Connect tab to link your ADP account</div>
    </div>
  )
    ...
    <div>
      ...
      <div style={s.card}>
        ...
        <button onClick={submit} disabled={submitting||employees.length===0} style={{ ...s.btn, opacity:(submitting||employees.length===0)?0.6:1 }}>
          <Icon name="send" size={15}/>{submitting?'SUBMITTING TO ADP...':'SUBMIT TO ADP'}
        </button>
        <div style={{ marginTop:'10px', fontSize:'11px', color:'var(--text-muted)' }}>
          Hours will be sent to ADP Workforce Now for payroll processing. ADP will calculate taxes and schedule direct deposits.
        </div>
      </div>
    </div>
  )
...
function PayStubsTab({ companyId, connection }) {
  ...
  async function viewStub(run) {
    if (!connection) { alert('Connect ADP Workforce Now to view pay stubs.'); return }
    try {
      const { data, error } = await supabase.functions.invoke('adp-proxy', {
        body: { action:'pay-stub-url', company_id:companyId, data:{ payroll_id:run.adp_payroll_id } }
      })
  ...
  if (!connection) return (
    <div style={{ textAlign:'center', padding:'60px 24px', color:'var(--text-muted)' }}>
      ...
      <div style={{ marginTop:'16px', fontSize:'15px' }}>Connect ADP Workforce Now to view pay stubs</div>
      <div style={{ marginTop:'6px', fontSize:'12px' }}>Go to the Connect tab to link your ADP account</div>
    </div>
  )
...
function SettingsTab({ companyId, connection, onSaved }) {
  ...
  async function syncEmployees() {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('adp-proxy', {
        body: { action:'sync_timesheets', company_id:companyId }
      })
      if (error) throw new Error(error.message)
      setMsg({ ok:true, text: data?.message || 'Employee data synced to ADP Workforce Now.' })
    } catch(e) {
  ...
  return (
    <div style={s.card}>
      <div style={{ fontSize:'11px', color:'var(--text-muted)', fontFamily:'var(--font-condensed)', textTransform:'uppercase', letterSpacing:'1.5px', marginBottom:'16px' }}>ADP Workforce Now Settings</div>
      ...
      <div style={{ marginBottom:'16px' }}>
        <div style={s.lbl}>ADP Company Code</div>
        <input style={{ ...s.inp, color:'var(--text-muted)', cursor:'not-allowed' }} value={connection?.company_code||'Not connected'} readOnly/>
      </div>
      ...
      {connection && (
        <button onClick={syncEmployees} disabled={syncing} style={{ ...s.btn, opacity:syncing?0.6:1 }}>
          <Icon name="refresh-cw" size={14}/>{syncing?'SYNCING...':'SYNC TIMESHEETS TO ADP'}
        </button>
      )}
      <div style={{ marginTop:'20px', padding:'14px', background:'var(--bg-surface)', borderRadius:'var(--radius-sm)', fontSize:'12px', color:'var(--text-muted)', lineHeight:1.6 }}>
        For help configuring payroll or troubleshooting your ADP Workforce Now connection, contact{' '}
        <a href="mailto:support@postcommand.app" style={{ color:'var(--accent)', textDecoration:'none' }}>support@postcommand.app</a>.
      </div>
    </div>
  )
