...
export default function ClockIn() {
  ...
  const E = (msg) => <div style={{background:'var(--color-danger-bg)',border:'1px solid rgba(224,85,85,0.3)',borderRadius:'var(--radius-md)',padding:'12px 16px',fontSize:'13px',color:'var(--color-danger)',marginBottom:'16px',display:'flex',gap:'10px',alignItems:'flex-start'}}><Icon name="alert-circle" size={16} style={{flexShrink:0}}/><span>{msg}</span></div>
  ...
  return (
    <div style={{padding:'24px',maxWidth:'480px',margin:'0 auto'}}>
      ...
      {step===STEPS.READY_IN && shift && site && <Card><SL>YOUR SHIFT</SL><ShiftSum shift={shift} site={site}/><div style={{marginTop:'24px'}}><button onClick={()=>{setStep(STEPS.PHOTO_IN);setTimeout(()=>photoInRef.current?.click(),100)}} style={pBtnStyle}><Icon name="log-in" size={18}/>CLOCK IN</button><p style={{fontSize:'12px',color:'var(--text-muted)',textAlign:'center',marginTop:'10px'}}>Photo required to complete clock-in</p></div></Card>}

      {step===STEPS.PHOTO_IN && <Card><SL>CLOCK-IN PHOTO</SL><div style={{textAlign:'center',padding:'24px 0'}}><Icon name="eye" size={40} color="var(--accent)"/><div style={{marginTop:'12px',fontSize:'14px',color:'var(--text-primary)',fontWeight:600}}>Take your clock-in photo</div><div style={{fontSize:'13px',color:'var(--text-muted)',marginTop:'6px'}}>Face forward, in uniform, at your post</div></div><input ref={photoInRef} type="file" accept="image/*" capture="environment" style={{display:'none'}} onChange={e=>handlePhotoCapture(e.target.files[0],'in')}/>{photoIn&&<div style={{marginBottom:'16px',borderRadius:'var(--radius-md)',overflow:'hidden',border:'1px solid var(--border-subtle)'}}><img src={photoIn} alt="" style={{width:'100%',maxHeight:'200px',objectFit:'cover',display:'block'}}/></div>}<button onClick={()=>photoInRef.current?.click()} style={pBtnStyle}><Icon name="eye" size={18}/>{photoIn?'RETAKE PHOTO':'OPEN CAMERA'}</button><button onClick={()=>setStep(STEPS.READY_IN)} style={{...sBtnStyle,marginTop:'8px'}}>CANCEL</button></Card>}

      {(step===STEPS.GPS_IN||step===STEPS.GPS_OUT) && <Card><div style={{textAlign:'center',padding:'32px'}}><div style={{animation:'pulse 1.5s infinite'}}><Icon name="map-pin" size={40} color="var(--accent)"/></div><div style={{marginTop:'16px',fontSize:'14px',color:'var(--text-primary)',fontWeight:600}}>Getting your location...</div><div style={{fontSize:'13px',color:'var(--text-muted)',marginTop:'6px'}}>Please allow location access when prompted</div></div></Card>}
      ...
      {step===STEPS.GEOFENCE_WARN && <Card>
        <div style={{background:'var(--color-warning-bg)',border:'1px solid rgba(232,148,58,0.3)',borderRadius:'var(--radius-md)',padding:'16px',marginBottom:'20px',display:'flex',gap:'12px',alignItems:'flex-start'}}>
          <Icon name="alert-circle" size={20} color="var(--color-warning)" style={{flexShrink:0}}/>
          <div><div style={{fontSize:'14px',fontWeight:700,color:'var(--color-warning)',marginBottom:'4px'}}>OUTSIDE GEOFENCE</div><div style={{fontSize:'13px',color:'var(--text-secondary)',lineHeight:1.5}}>You are <strong>{distance}m</strong> from <strong>{site?.name}</strong>. Allowed radius: <strong>{Number(site?.geofence_radius)||DEFAULT_GEOFENCE_RADIUS}m</strong>.</div></div>
        </div>
        ...
        <div style={{fontSize:'12px',color:'var(--text-muted)',marginBottom:'16px',padding:'10px',background:'var(--bg-card)',borderRadius:'var(--radius-sm)',border:'1px solid var(--border)'}}><Icon name="bell" size={13} style={{verticalAlign:'middle',marginRight:'6px'}}/>Your supervisor will be notified of this override</div>
      ...
      {step===STEPS.COMPLETE && timesheet && <Card>
        <div style={{textAlign:'center',padding:'16px 0 24px'}}>
          ...
          <div style={{fontSize:'13px',color:'var(--text-muted)'}}>Your timesheet has been submitted for review</div>
        </div>
      ...
      <div style={{marginTop:'16px',fontSize:'11px',color:'var(--text-muted)',textAlign:'center'}}>Timesheet photos are securely stored and only accessible to authorized personnel</div>
    </div>
  )
}
...
const pBtnStyle = { width:'100%', height:'52px', background:'var(--accent)', border:'none', borderRadius:'var(--radius-md)', color:'var(--text-inverse)', fontFamily:'var(--font-display)', fontSize:'18px', letterSpacing:'2px', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'10px', transition:'all 150ms ease' }
